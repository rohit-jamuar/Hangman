...
def welcome():
    '''
    -- Welcome page!
    -- This function is the endpoint with which a player interacts with. 
...
def hello():
    ...
    -- This function also ensures that a player is unable to cheat the game - should the
       player intend to refresh the game mid-stream, it would be counted as a loss.
    -- All the GET requests are made by internal sources : there is a verification
...
def hangman_game(name):
	...
	--- Once a player clicks on available set of characters, a POST request is made to this endpoint.
        ...
        becomes the "answer" or the total number of available chances (i.e. "chance_number") is exhausted,
        the execution redirects itself to the "/index" endpoint. Also, "new_game" variable maintains the
        current state of game. This variable ensures that a user is unable to forfeit the game midway.
    '''
